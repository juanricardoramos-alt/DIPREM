-- ============================================================================
-- DIPREM CRM — ACTUALIZACIÓN: Mercado nacional + análisis de gestión
-- Para proyectos que ya corrieron setup_completo.sql (+ actualizacion_fase3.sql).
...
create table if not exists proyectos_mercado (
...
create unique index if not exists proyectos_mercado_unicos
  on proyectos_mercado (lower(trim(nombre)), lower(trim(empresa)));
create index if not exists proyectos_mercado_estado_idx   on proyectos_mercado (estado);
create index if not exists proyectos_mercado_asignado_idx on proyectos_mercado (asignado_a);
...
drop policy if exists proyectos_mercado_select on proyectos_mercado;
...
drop policy if exists proyectos_mercado_insert on proyectos_mercado;
...
drop policy if exists proyectos_mercado_update on proyectos_mercado;
create policy proyectos_mercado_update on proyectos_mercado for update to authenticated
  using (public.es_admin() or public.rol_actual() = 'gerente');
drop policy if exists proyectos_mercado_delete on proyectos_mercado;
create policy proyectos_mercado_delete on proyectos_mercado for delete to authenticated
  using (public.es_admin() or public.rol_actual() = 'gerente');

drop trigger if exists proyectos_mercado_auditoria on proyectos_mercado;
...
-- ---------------------------------------------------------------------------
create or replace function public.importar_proyectos_mercado(p_proyectos jsonb)
...
-- ---------------------------------------------------------------------------
create or replace function public.marcar_proyecto_convertido()
...
drop trigger if exists leads_marcar_proyecto on leads;
...
-- ---------------------------------------------------------------------------
alter table actividades add column if not exists lead_id uuid references leads(id);
create index if not exists actividades_lead_idx on actividades (lead_id);
...
drop trigger if exists actividades_gestion_lead on actividades;
...
create or replace view public.v_distribucion_gestion
with (security_invoker = true) as
select
  a.propietario_id,
  u.nombre  as ejecutivo,
...
-- ✅ Listo. Verifica con:
--   select count(*) from proyectos_mercado;
--   select * from v_gestion_contactos limit 5;
