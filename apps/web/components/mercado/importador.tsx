  ...
  return (
    <Dialogo abierto={abierto} titulo={es.mercado.importarTitulo} onCerrar={cerrar}>
      <div className="space-y-4">
        <p className="text-sm text-slate-600">{es.mercado.formatoAyuda}</p>

        <label className="block cursor-pointer rounded-lg border-2 border-dashed border-slate-300 p-6 text-center text-sm text-slate-500 hover:border-[var(--color-diprem)]">
          <input
            type="file"
            accept=".csv,.xlsx,.xls"
            className="hidden"
        ...
        {analizando && <p className="text-sm text-slate-400">{es.mercado.analizando}</p>}
                ...
                <table className="mt-2 w-full text-xs">
                  ...
                  <tbody>
                    {validacion.validos.slice(0, 5).map((p) => (
                      <tr key={`${p.nombre}-${p.empresa}`} className="border-t border-slate-200">
                    ...
                    {validacion.validos.length > 5 && (
                      <tr className="border-t border-slate-200 text-slate-400">
