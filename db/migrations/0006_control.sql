  ...
  loop
    ...
    update proyectos_mercado
       set estado = 'asignado',
           ...
           asignado_en = now(),
           asignado_por = public.usuario_actual(),
           lead_id = v_lead
...
-- (grant select a authenticated: movido a la migración de seguridad)
...
-- ---------------------------------------------------------------------------
create function public.recordar_proyecto(p_proyecto_id uuid)
